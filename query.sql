...
from dept_manager dm inner join departments as d on dm.dept_no = d.dept_no 
				     inner join employees as e on dm.emp_no = e.emp_no;

---4.List the department of each employee with the following information: 
---employee number, last name, first name, and department name.
create view latest_dept_emp as
select distinct on (emp_no) emp_no,dept_no,from_date,to_date
from dept_emp 
order by emp_no,from_date desc

select e.emp_no, e.last_name, e.first_name,d.dept_name
from employees e inner join latest_dept_emp de   on e.emp_no = de.emp_no
...
select e.emp_no, e.last_name, e.first_name, d.dept_name
from employees e left join latest_dept_emp de on e.emp_no=de.emp_no
